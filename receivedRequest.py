...
cursor = db.cursor()
query = "SELECT title, e_date, e_time, sender FROM events WHERE receiver = %s"
...
if received_events:
    ...
    for event in received_events:
        ...
        table_html += f'<p class="box-content"><strong>Request Person:</strong> {sender}</p>'
        table_html += f'<button class="accept-button" onclick="acceptEvent(this)" data-title="{title}" data-sender="{sender}">Accept</button>'
        table_html += '</div>'
